...
fn interpolateV(va: vec4<f32>, vb: vec4<f32>) -> vec4<f32> {
    ...
    elseif (abs(va.w-vb.w) < 0.00001) {
     ...
     // non-interpolated  if (abs(isovalue - va.w) < 0.0001) {
     // non-interpolated     return vec4(va.xyz, 1.0);
     // non-interpolated  }
     // non-interpolated  else if (abs(isovalue - vb.w) < 0.0001) {
     // non-interpolated     return vec4(vb.xyz, 1.0);
     // non-interpolated  }
     // non-interpolated  else if (abs(va.w-vb.w) < 0.0001) {
     // non-interpolated     return vec4(va.xyz, 1.0);
     // non-interpolated  }
     // non-interpolated  return vec4(va.xyz, 1.0);
}
...
fn interpolateN(na: vec4<f32>, nb: vec4<f32>, densityA: f32, densityB: f32) -> vec4<f32> {
    ...
    elseif (abs(densityA-densityB) < 0.00001) {
      ...
      return vec4<f32>(normalize(vec3<f32>(x, y, z)), 0.0);
       ...
       // non-interpolated  if (abs(isovalue - densityA) < 0.000001) { return vec4(normalize(na.xyz), 0.0); }
       // non-interpolated  else if (abs(isovalue - densityB) < 0.000001) { return vec4(normalize(nb.xyz), 0.0);}
       // non-interpolated  else if (abs(densityA-densityB) < 0.000001) { return vec4(normalize(na.xyz), 0.0); }
       // non-interpolated  float iso = (densityA + densityB) * 0.5;
       // non-interpolated  float mu = (iso - densityA) / (densityB - densityA);
       // non-interpolated  float x = na.x + mu * (nb.x - na.x);
       // non-interpolated  float y = na.y + mu * (nb.y - na.y);
       // non-interpolated  float z = na.z + mu * (nb.z - na.z);
       // non-interpolated  return vec4(normalize(vec4(x, y, z, 0.0)));
}

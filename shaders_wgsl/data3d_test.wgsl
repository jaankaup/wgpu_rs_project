...
fn cnoise(P: vec4<f32>) -> f32 {
  //let Pi0 = modf(floor(P), rep); // Integer part modulo rep
  //let Pi1 = modf(Pi0 + 1.0, rep); // Integer part + 1 mod rep
  //let Pf0 = fract(P); // Fractional part for interpolation
  //let Pf1 = Pf0 - 1.0; // Fractional part - 1.0
  var Pi0 = floor(P); // Integer part for indexing
  var Pi1 = Pi0 + 1.0; // Integer part + 1
  // Pi0 = modf(Pi0, vec4<f32>(289.0));
  // Pi1 = modf(Pi1, vec4<f32>(289.0));
...
fn main([[builtin(local_invocation_id)]] local_id: vec3<u32>,
  ...
  //let noise_c = cnoise(vec4<f32>(vec3<f32>(fz, fy, fx) + noise_velocity*future_usage1.x, 1.0));
  let noise_c = cnoise(vec4<f32>(vec3<f32>(fz, fy, fx) + vec3<f32>(noise_velocity*future_usage1.something.x), 1.0));
